...
/// **Note**
/// Debug: 需要打印
/// PartialOrd: (结构体的部分字段可比较) OrderEvent是需要按照time排序并处理
/// Serialize, Deserialize: 该类型需要序列化(反序列化)成字节以便存储和网络传输(何时网络传输?)
#[derive(Debug, PartialOrd, Serialize, Deserialize)]
...
///
/// TODO: 是否需要在该枚举类型的变体中，内嵌对应的数据结构体
/// TODO: trait还可能需要增加
///
/// **Note**
/// Copy: 赋值的时候，隐式的自动复制具有所有权值
/// Debug: 打印
/// Eq: ?
/// Ord: ?
/// Hash: ?
/// Serialize, Deserialize: 该类型需要序列化(反序列化)成字节以便存储和网络传输(何时网络传输?)
#[derive(Copy, Debug, Eq, Ord, Hash, Serialize, Deserialize)]
pub enum OrderKind {
